...
async function loadBackendPlayer() {
  ...
  state.team = {
    ...
    themeColor: data.player.teamThemeColor || data.player.team_theme_color || "",
  };
...
function loadLocalPlayer() {
  ...
  applyTeamTheme(team?.themeColor || "");
}
  ...
  try {
    if (mode === "backend") {
      ...
      if (preferBackendOnLocalhost) {
        throw new Error("Backend player session required on localhost.");
      }
      try {
        loadLocalPlayer();
      } catch {
